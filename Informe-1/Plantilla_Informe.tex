...
Inicialmente, se utilizaron las ecuaciones \eqref{Ohm} y \eqref{nodos} para resolver el circuito mediante el método de ramas y así obtener el valor de las corrientes $i_1$, $i_2$ e $i_3$ en base a los parámetros. 
...
Una vez hecho esto, fue posible conectar una resistencia de carga $R_q = R_v + R_A$ representada por una resistencia variable por decadas en serie con el multimetro en modo amperimetro. El objetivo fue medir la corriente $I$ para distintos valores de $R_v$ y en base a ambas magnitudes calcular la potencia $P$ disipada por $R_q$ a través de \eqref{Joule} buscando relevar la curva acampanada $P(R_q)$.
...
Luego, con las mediciones correspondientes a un voltaje constante, se decidio realizar un grafico que relacionara las corrientes medidas con el inverso de las resistencias seleccionadas, de esa forma, si se cumple \eqref{Ohm}, deberia haber una correspondencia lineal entre esos pares de valores. La \textbf{Figura \ref{fig:Ohm_hip}} confirma lo esperado, ya que una vez realizado un ajuste lineal, arroja un $R-square = 0,99998$.
...
De los datos obtenidos se tiene el gráfico de la \textbf{Figura \ref{fig:pot_res}} donde se puede observar que el comportamiento de la potencia entregada por el circuito esta dentro de los parametros esperados. Luego, ajustando los puntos por una curva de la forma de \eqref{Pot}, se obtiene una resistencia de máxima disipación $R_{max} = (78.5 \pm 0.8)\Omega$ y un voltaje $\varepsilon_{th} = (0,74\pm 0,01)V$. Recordando que según \eqref{Pot} resulta $R_{max} = R_{th}$, puede verse que ambos resultados son muy cercanos a los valores de Thevenin calculados anteriormente. Sin embargo, el coeficiente $\chi_{red}^{2} = 0.01362$ muestra una enorme sobreestimación de los errores, visible ya en la \textbf{Figura \ref{fig:pot_res}}.
...
Finalmente, se calculó la eficacia con la cual se traspasaba la potencia del circuito hacia la carga. En el grafico de la \textbf{Figura \ref{fig:efi_res}} se ve que hay una tendencia creciente pero que en los ultimos puntos empieza a dispersarse. 
